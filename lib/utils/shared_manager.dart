...
class SharedManager {
  ...
  bool isFacebookLoginEnabled(SharedPreferences? preferences) {
    return preferences!.getBool(Constants.facebookLoginConfig) ??
        Setup.isFacebookLoginEnabled;
  }

  bool isAppleLoginEnabled(SharedPreferences? preferences) {
    return preferences!.getBool(Constants.appleLoginConfig) ??
        Setup.isAppleLoginEnabled;
  }

  bool isPhoneLoginEnabled(SharedPreferences? preferences) {
    return preferences!.getBool(Constants.phoneLoginConfig) ??
        Setup.isPhoneLoginEnabled;
  }

  int getDiamondsEarnPercent(SharedPreferences? preferences) {
    return preferences!.getInt(Constants.diamondsEarnPercentConfig) ??
        Setup.diamondsEarnPercent;
  }

  int getWithDrawPercent(SharedPreferences? preferences) {
    return preferences!.getInt(Constants.withDrawPercentConfig) ??
        Setup.withDrawPercent;
  }

  int getAgencyPercent(SharedPreferences? preferences) {
    return preferences!.getInt(Constants.agencyPercentConfig) ??
        Setup.agencyPercent;
  }

  int getDiamondsNeededToRedeem(SharedPreferences? preferences) {
    return preferences!.getInt(Constants.diamondsNeededToRedeemConfig) ??
