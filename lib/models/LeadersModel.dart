...
class LeadersModel extends ParseObject implements ParseCloneable {
  ...
  @override
  LeadersModel clone(Map<String, dynamic> map) =>
      LeadersModel.clone()..fromJson(map);
  ...
  set setCounterDiamondsQuantity(int count) =>
      set<int>(keyDiamondsQuantity, count);
  set incrementDiamondsQuantity(int count) =>
      setIncrement(keyDiamondsQuantity, count);

  GiftsSentModel? get getGiftsSent => get<GiftsSentModel>(keyGiftsSent);
  set setGiftsSent(GiftsSentModel giftsSent) =>
      addRelation(keyGiftsSent, [giftsSent]);
}
