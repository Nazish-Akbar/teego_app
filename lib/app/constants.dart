...
class Constants {
  ...
  static bool isSelfHosted =
      Constants.getCloudParse.contains(Config.serverUrl) ? false : true;
  ...
  static void queryParseConfig(SharedPreferences prefs) async {
    ...
    if (response.success) {
      ...
      prefs.setBool(phoneLoginConfig,
          phoneLogin != null ? phoneLogin : Setup.isPhoneLoginEnabled);
      prefs.setBool(appleLoginConfig,
          appleLogin != null ? appleLogin : Setup.isAppleLoginEnabled);
      prefs.setBool(googleLoginConfig,
          googleLogin != null ? googleLogin : Setup.isGoogleLoginEnabled);

      prefs.setBool(callsConfig, calls != null ? calls : Setup.isCallsEnabled);
      prefs.setString(
          streamTypeConfig,
          steamingProvider != null
              ? steamingProvider
              : Setup.streamingProviderType);
      prefs.setString(
          streamKeyConfig,
          streamerProviderKey != null
              ? streamerProviderKey
              : Setup.streamingProviderKey);

      prefs.setBool(
          withdrawPaypalConfig,
          withdrawPaypal != null
              ? withdrawPaypal
              : Setup.isWithdrawPaypalEnabled);
      prefs.setBool(
          withdrawPayoneerConfig,
          withdrawPayoneer != null
              ? withdrawPayoneer
              : Setup.isWithdrawPayoneerEnabled);
      prefs.setBool(withdrawIbanConfig,
          withdrawIban != null ? withdrawIban : Setup.isWithdrawIbanEnabled);
      prefs.setBool(withdrawUSDTConfig,
          withdrawUsdt != null ? withdrawUsdt : Setup.isWithdrawUSDTlEnabled);

      prefs.setInt(agencyPercentConfig,
          agencyPercent != null ? agencyPercent : Setup.agencyPercent);
      prefs.setInt(
          diamondsEarnPercentConfig,
          diamondsEarnPercent != null
              ? diamondsEarnPercent
              : Setup.diamondsEarnPercent);
      prefs.setInt(
          diamondsNeededToRedeemConfig,
          diamondsNeededToRedeem != null
              ? diamondsNeededToRedeem
              : Setup.diamondsNeededToRedeem);
      prefs.setInt(withDrawPercentConfig,
          withDrawPercent != null ? withDrawPercent : Setup.withDrawPercent);
      ...
      prefs.setString(
          s3SecretKeyConfig, s3SecretKey != null ? s3SecretKey : "");
      prefs.setString(
          s3AccessKeyConfig, s3AccessKey != null ? s3AccessKey : "");

      prefs.setBool(
          bannerAdsOnHomeReelsEnabledConfig,
      ...
      if (prefs.getInt(diamondsNeededToRedeemConfig) == null)
        prefs.setInt(
            diamondsNeededToRedeemConfig, Setup.diamondsNeededToRedeem);
